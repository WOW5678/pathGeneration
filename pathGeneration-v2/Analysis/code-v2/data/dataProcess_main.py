...
import math
import operator
from get_discharge_summaries import write_discharge_summaries
...
if __name__ == '__main__':
    ...
    dfdiag=pd.read_csv(open('%s\DIAGNOSES_ICD.csv\DIAGNOSES_ICD.csv'%mimic_iii_dir_1,'r'),names=[ 'ROW_ID','SUBJECT_ID','HADM_ID','SEQ_NUM','ICD9_CODE'])
    ...
    # #len(dfcodes) 891142
    dfdiag.to_csv('ALL_CODES.csv', index=False,
    ...
    # lowercase all token
    disch_full_file = write_discharge_summaries(out_file='disch_full.csv')
    dfdisch_full = pd.read_csv('disch_full.csv',
    ...
    dfcodes=dfdiag.sort_values(['SUBJECT_ID', 'HADM_ID'])
    ...
    print(hadm_ids)
    with open('ALL_CODES.csv', 'r') as lf:
        with open('ALL_CODES_filtered.csv', 'w',newline='') as of:
            w = csv.writer(of)
    ...
    dfcodes_filtered=pd.read_csv(open('ALL_CODES_filtered.csv'),index_col=None)
    ...
    dfcodes_filtered.to_csv('ALL_CODES_filtered.csv', index=False)

    #Append labels to notes in a single file

    # Now let's append each instance with all of its codes
    # this is pretty non-trivial so let's use this script I wrote, which requires the notes to be written to file
    sorted_file = 'disch_full.csv'
    dfdisch_full.to_csv(sorted_file, index=False)

    labeled=concat_and_split.concat_data('ALL_CODES_filtered.csv', sorted_file)
    ...
    #train,dev,test=concat_and_split.split_data(labeled,base_name='disch')
