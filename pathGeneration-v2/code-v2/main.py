    ...
    while epoch <max_run_epoch:
        ...
        while step<len(batches):
            ...
            for sample,label in zip(batchPathes,labels):
                ...
                #print('sample:',sample)
                print('pred:', pred,[args.id2node.get(item) for item in pred])
                print('label:',label,[args.id2node.get(item) for item in label])
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        for step in range(int(len(batches)/1000)):
            ...
            # 训练 D网络
            train_discriminator(d_model,5,d_batcher,d_batcher.get_batches(mode="train"))
